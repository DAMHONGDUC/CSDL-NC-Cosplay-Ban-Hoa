...
-- LịCH SỬ XUẤT
-- PARTITION CHO BẢNG DONHANG

-- Create filegroup for database
ALTER DATABASE HYT ADD FILEGROUP FileGroup_LSX_2020
ALTER DATABASE HYT ADD FILEGROUP FileGroup_LSX_2021
...
Alter Database HYT
 ADD File 
 (
 Name = N'FileGroup_LSX_2020',
 FileName = N'C:\Program Files\Microsoft SQL Server\MSSQL15.MSSQLSERVER\MSSQL\DATA\FileGroup_LSX_2020.ndf',
	SIZE = 80MB,
	MAXSIZE = 2200,
	FILEGROWTH = 1024MB 
 )
 To FileGroup FileGroup_LSX_2020;
 GO
 
 Alter Database HYT
 ADD File 
 (
 Name = N'FileGroup_LSX_2021',
 FileName = N'C:\Program Files\Microsoft SQL Server\MSSQL15.MSSQLSERVER\MSSQL\DATA\FileGroup_LSX_2021.ndf',
	SIZE = 80MB,
	MAXSIZE = 2200,
	FILEGROWTH = 1024MB 
 )
 To FileGroup FileGroup_LSX_2021;
 GO
  --- Create partition function
 USE HYT
 CREATE PARTITION FUNCTION LSX_PartitionFunction (DATE)
 AS RANGE RIGHT FOR VALUES ('2021-1-1')
GO

-- Create partition schema
CREATE PARTITION SCHEME LSX_PartitionSchema
AS PARTITION LSX_PartitionFunction TO ( [PRIMARY],FileGroup_LSX_2020,
FileGroup_LSX_2021, [PRIMARY])
GO
...
-- Đầu tiên là phải xóa index dc SQL tạo tự động trước
ALTER TABLE dbo.DONHANG DROP PK_DH

-- Sau đó mới chạy đoạn script sau
CREATE CLUSTERED INDEX [PK_DH]
ON  [dbo].[DONHANG](NGAYLAP)
ON  [LSX_PartitionSchema](NGAYLAP)
GO 