...
BEGIN
	SELECT LSN.MASP,LSN.NGAYNHAP,NV.TENNV,LSN.SOLUONG, LSN.GIANHAP FROM LICHSUNHAP LSN, NHANVIEN NV
	WHERE LSN.NGUOINHAP = NV.MANV AND
	LSN.NGAYNHAP = @NGAYNHAP 
	ORDER BY LSN.NGAYNHAP DESC
END
GO
--
...
BEGIN
	SELECT SP.MASP,SP.TENSP,SP.GIAGOC,SP.KHUYENMAI, GG.GIAGIAM,SP.HINHANH FROM SANPHAM SP, GIAMGIA GG
	WHERE SP.MASP=GG.MASP AND SP.TENSP like @TENSP +'%'
...
AS
	BEGIN
		...
		FROM SANPHAM SP
		WHERE SP.MASP IN (
			SELECT TOP(30) PERCENT WITH TIES SP1.MASP
...
AS
	BEGIN
		SELECT TOP(30) PERCENT WITH TIES SUM(CT.SOLUONG) AS SLDB
		FROM SANPHAM SP1, DONHANG DH, CT_DONHANG CT
		WHERE SP1.MASP = CT.MASP AND DH.MADH = CT.MADH AND DH.TINHTRANG = 2 AND MONTH(DH.NGAYLAP) = @THANG AND YEAR(DH.NGAYLAP) = @NAM
		GROUP BY SP1.MASP
		ORDER BY SUM(CT.SOLUONG) DESC
...
AS
	BEGIN
		SELECT TOP(30) PERCENT WITH TIES SUM(CT.THANHTIEN) - (SUM(CT.SOLUONG) * LS.GIANHAP) AS DT
